...
cat >bundle.yml <<HERE
---
name: http://example.org/yep#abundle
description: |-
...
owm bundle list
# Output:
# ! http://example.org/yep#abundle - Cannot find bundle descriptor at 'bundle.yml'
...
owm bundle deregister http://example.org/yep#abundle
...
# [FETCH]
# That's fine for creating and listing bundles, but what about when you want someone
# else's bundles? To fetch a bundle, you need the bundle name. That name is queried for in
# your local repository (see below in [LOCAL]), then in any remotes configured in your
# ".owm" directory or user settings, and finally the default remote (see [REMOTE]). `owm bundle fetch`
# does this for you.
owm bundle fetch https://openworm.org/data/bundles/example_bundle.00
...
from owmeta.dataObject import TypeDataObject, DataObject, RDFSSubClassOfProperty

# Use the bundle
with Bundle('https://openworm.org/data/bundles/example_bundle.01') as bnd:
    # "contextualize" the Context with the bundle to access contexts within the bundle
    ctx = bnd(Context)('https://openworm.org/data#example_bundle_context')
...
# a bundle https://openworm.org/data/bundles/example_bundle.01 which we had not previously
...
# Output:
# https://openworm.org/data/bundles/example_bundle.00 - An example bundle
# https://openworm.org/data/bundles/example_bundle.01 - Another example bundle

# To remove a bundle from the index:
owm bundle index remove https://openworm.org/data/bundles/example_bundle.00
