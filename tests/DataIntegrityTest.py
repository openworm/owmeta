...
class Test(unittest.TestCase):
    ...
    def test_compare_to_xls(self):
        ...
        global AVAL_POST_CELL_QUERY, AVAL_PRE_CELL_QUERY
        SAMPLE_CELL = 'AVAL'
        xls_conns = set([])

        # QUERY TO GET ALL CONNECTIONS WHERE SAMPLE_CELL IS ON THE PRE SIDE
        aval_pre_conns = self.retrieve_aval_post_neighbors()

        # QUERY TO GET ALL CONNECTIONS WHERE SAMPLE_CELL IS ON THE *POST* SIDE
        aval_post_conns = self.retrieve_aval_pre_neighbors()

        pow_conns = set(aval_pre_conns) + set(aval_post_conns)
        ...
        for row in range(1, s.nrows):
            if s.cell(row, 2).value in ('S', 'Sp', 'EJ') and \
                ...
                pre = self.normalize(s.cell(row, 0).value)
                post = self.normalize(s.cell(row, 1).value)
    ...
    def retrieve_aval_post_neighbors(self):
        qres = self.g.query(AVAL_PRE_CELL_QUERY.format(name=R.Literal('AVAL').n3()))

        return [('AVAL',) + tuple(str(x.value) for x in line) for line in qres]

    def retrieve_aval_pre_neighbors(self):
        qres = self.g.query(AVAL_POST_CELL_QUERY.format(name=R.Literal('AVAL').n3()))
        aval_post_conns = [tuple(str(x.value) for x in line) for line in qres]
        for t in aval_post_conns:
            t.insert(1, 'AVAL')
        return aval_post_conns
...
AVAL_PRE_CELL_QUERY = """
SELECT ?post_name ?type (STR(?num) AS ?numval) WHERE {{
   #############################################################
...
AVAL_POST_CELL_QUERY = """
SELECT ?pre_name ?type (STR(?num) AS ?numval) WHERE {{
   #############################################################
