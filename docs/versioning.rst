...
Software Versioning
===================
The |pow| library follows the `semanitc versioning scheme
<https://semver.org>`_. For the sake of versioning, the software interface
consists of:

#. the `pow` command line interface, 
#. the underlying `PyOpenWorm.command.POW` class underlying that CLI,
#. all "public" definitions in the `PyOpenWorm` package, sub-packages, and
   sub-modules,
#. the format of RDF data generated by `PyOpenWorm.dataObject.DataObject` and
   the subclasses thereof defined in the `PyOpenWorm` package, sub-packages, and
   sub-modules,
#. the API documentation for the `PyOpenWorm` package, sub-packages, and
   sub-modules,
