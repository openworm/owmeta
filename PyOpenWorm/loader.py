...
def download_torrent(torrent_name):
    ...
    with open("../.pow/credentials.json", "r") as jsonFile:
        data = json.load(jsonFile)

    tmp = data["installed"]['client_secret']
    print("--")
    print(tmp)
    key = input("The key")
    data["installed"]['client_secret'] = decrypt(key, "")

    with open("../.pow/credentials.json", "w") as jsonFile:
    ...
    # time.
    if os.path.exists('../.pow/token.pickle'):
        with open('../.pow/token.pickle', 'rb') as token:
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                '../.pow/credentials.json', SCOPES)
            creds = flow.run_local_server()
        # Save the credentials for the next run
        with open('../.pow/token.pickle', 'wb') as token:
